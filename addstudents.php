...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Remove dashes from student ID before validation
    $clean_student_id = str_replace('-', '', $students_id);

    // Validate Student ID - exactly 6 digits
    if (!preg_match('/^\d{6}$/', $clean_student_id)) {
        echo "<script>
        ...
        try {
            ...
            $stmt->bind_param(
                ...
                $clean_student_id, $user_id, $_POST['FirstName'], $_POST['LastName'], $_POST['MI'], $_POST['Suffix'],
...
function formatStudentId(input) {
    ...
    // Validate the clean version (without dashes)
    const cleanValue = value.replace(/\D/g, '');
    validateStudentId(cleanValue);
}

// Student ID validation - exactly 6 digits
function validateStudentId(cleanStudentId) {
    const studentIdInput = document.querySelector('input[name="students_id"]');
    
    if (cleanStudentId && cleanStudentId.length === 6) {
        studentIdInput.style.borderColor = '#10b981';
        return true;
    } else if (cleanStudentId && cleanStudentId.length > 0) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add event listeners for student ID formatting
    studentIdInput.addEventListener('input', function() {
        formatStudentId(this);
    });
    
    studentIdInput.addEventListener('keydown', handleStudentIdKeydown);
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Set the clean student ID value for form submission
        const hiddenStudentId = document.createElement('input');
        hiddenStudentId.type = 'hidden';
        hiddenStudentId.name = 'students_id';
        hiddenStudentId.value = cleanStudentId;
        ...
        <section>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="students_id" required placeholder="Stundents ID" 
                           class="student-id-input" maxlength="7">
                    <div class="student-id-format">
                        <i class="fa-solid fa-info-circle"></i>
                        Format: </span> (6 digits total)
                    </div>
                </div>
                <div class="form-group">
