<?php
session_start();
if (!isset($_SESSION['username']) || !in_array($_SESSION['usertype'], ['Senator', 'Social Manager', 'Governor', 'Vice Governor'])) {
...
/* 🎯 SETTINGS BUTTON - BLACK ICON, NO BACKGROUND */
.settings-btn {
...
<div class="container">
    ...
    <div class="event-header">
        <div class="event-title-container">
            <h1 class="event-title">Event Name: <?= htmlspecialchars($event_name) ?></h1>
    ...
    <form method="POST" action="" id="attendanceForm">
        <div class="attendance-form">
            <div class="form-header">
                ...
                <div class="real-time-clock" id="realTimeClock">
                    <!-- Real-time clock will be updated by JavaScript -->
                </div>
            </div>
            ...
            <div class="table-container">
                <table>
                    ...
                    <?php if ($students_result->num_rows > 0): ?>
                        ...
                        ?>
                        <tr>
                            <!-- STUDENT ID CELL REMOVED -->
                            <td><strong><?= htmlspecialchars($student['LastName'] . ', ' . $student['FirstName']) ?></strong></td>
                            <td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="<?= ($show_am && $show_pm) ? 6 : 4 ?>">
        ...
        <!-- FORM ACTIONS - FIXED AT BOTTOM RIGHT -->
        <div class="form-actions">
            <!-- Settings Dropdown -->
            <div class="settings-dropdown" id="settingsDropdown">
                <button type="button" class="settings-item" onclick="goBackToView()">
                    <i class="fa fa-arrow-left"></i>
                    Back to View Attendance
                </button>
                <button type="button" class="settings-item" onclick="saveAllChanges()">
            ...
            <!-- 🎯 SETTINGS BUTTON - BLACK ICON, NO BACKGROUND -->
            <button type="button" class="btn settings-btn" id="settingsButton">
                <i class="fa fa-cog"></i>
            </button>
        </div>
    </form>
...
// 🎯 GET CURRENT TIME in 24-hour format (HH:MM) for HTML time input
function getCurrentTime() {
    const now = new Date();
    let hours = now.getHours();
    let minutes = now.getMinutes();
    
    // Add leading zeros (24-hour format required for <input type="time">)
    hours = hours < 10 ? '0' + hours : hours;
    minutes = minutes < 10 ? '0' + minutes : minutes;
    
    return `${hours}:${minutes}`; // Returns "08:39" or "14:25"
}

// 🎯 STUDENT ID FORMATTING WITH AUTO-DASH
function formatStudentId(input) {
    ...
    // Add dash after 2 digits (format: XX-XXXX)
    if (value.length > 2) {
    ...
    // Return clean version (without dashes) for validation
    return value.replace(/\D/g, '');
}

// 🎯 HANDLE BACKSPACE AND DELETE KEYS PROPERLY
function handleStudentIdKeydown(e) {
    ...
    if (e.key === 'Backspace' && cursorPosition === 3 && value.charAt(2) === '-') {
        e.preventDefault();
        // Move cursor back one position and delete the digit before dash
        input.setSelectionRange(cursorPosition - 1, cursorPosition - 1);
        input.value = value.substring(0, cursorPosition - 2) + value.substring(cursorPosition);
    }
    
    // If delete is pressed and cursor is right before a dash
    if (e.key === 'Delete' && cursorPosition === 2 && value.charAt(2) === '-') {
        e.preventDefault();
        // Delete the digit after the dash
        input.value = value.substring(0, cursorPosition) + value.substring(cursorPosition + 2);
...
// 🎯 CHECK IF TIME IS VALID FOR SESSION - FIXED LOGIC
...
function getSessionStatusMessage(inputName, currentHour) {
    if (inputName.includes('amLogin') || inputName.includes('amLogout')) {
        if (currentHour < 5) {
            return "AM attendance will open at 5:00 AM.";
        } else {
            return "AM attendance has already closed. Sorry.";
        }
    } else if (inputName.includes('pmLogin') || inputName.includes('pmLogout')) {
        if (currentHour < 12) {
            return "PM attendance will open at 1:00 PM.";
        } else {
            return "PM attendance will open at at 1:00. Please wait.";
        }
    }
    return "This session is not available at the moment.";
}

async function verifyAndRecordTime(input) {
    // DOUBLE CHECK: If input already has value OR is marked as recorded, DO NOTHING
    if (input.value || input.dataset.recorded === 'true') {
        return;
    }
    ...
    if (!isValidTimeForSession(input.name, currentHour)) {
        ...
        const statusMessage = getSessionStatusMessage(input.name, currentHour);
        
        // Get current time in 12-hour format for display only
        ...
        await Swal.fire({
            ...
            html: `
                ...
                <p style="background: #fee2e2; padding: 12px; border-radius: 8px; color: #991b1b;">
                    <i class="fa fa-clock"></i>
                    Current time: <strong>${displayTime}</strong>
                </p>
    ...
    const { value: enteredID } = await Swal.fire({
        ...
        padding: '40px',
        customClass: {
            input: 'swal-input-extra-large student-id-input'
        },
        didOpen: () => {
            const swalInput = document.querySelector('.swal-input-extra-large');
            if (swalInput) {
                swalInput.addEventListener('input', function() {
                    formatStudentId(this);
                });
                swalInput.addEventListener('keydown', handleStudentIdKeydown);
            }
        },
        inputValidator: (value) => {
            ...
            if (cleanValue.length !== 6) {
                return 'Student ID must be 6 digits (format: XX-XXXX)!';
            }
        }
    });
    ...
    if (enteredID) {
        const cleanEnteredID = enteredID.replace(/\D/g, '');
        const cleanCorrectID = correctStudentID.replace(/\D/g, '');
        
        if (cleanEnteredID === cleanCorrectID) {
            // ✅ CORRECT ID - Get time in 24-hour format for input
            const currentTime24 = getCurrentTime(); // Returns "08:39" or "14:25"
            ...
            // Set the value in 24-hour format (required by HTML time input)
            input.value = currentTime24;
            input.classList.add('verified', 'has-record');
            input.style.cursor = 'not-allowed';
            input.readOnly = true;
            input.dataset.recorded = 'true';
            
            console.log('Time recorded:', currentTime24); // Debug log
            
            await Swal.fire({
                icon: 'success',
                title: 'Recorded Successfully!',
                html: `
                    <p>Student ID <strong>${enteredID}</strong> verified!</p>
                    <p style="margin-top: 10px; background: #d1fae5; padding: 10px; border-radius: 8px;">
                        <i class="fa fa-clock" style="color: #065f46;"></i>
                        Time recorded: <strong style="color: #065f46;">${displayTime}</strong>
                    </p>
                `,
                timer: 2000,
